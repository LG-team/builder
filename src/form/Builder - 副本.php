...
class Builder extends ZBuilder
{
    ...
    {
		$this->_template   =\think\facade\Env::get('app_path').'common/builder/form/layout.html';
    ...
    private function loadFile($type = '', $files_name = '', $module = '')
    {
        if ($files_name != '') {
            ...
            foreach ($files_name as $item) {
                if (strpos($item, '/')) {
                    $this->_vars[$type.'_list'][] = \think\facade\Env::get('root_path'). '/public/static/'. $item.'.'.$type;
                } else {
                    $this->_vars[$type.'_list'][] = \think\facade\Env::get('root_path'). '/public/static/'. $module .'/'.$type.'/'.$item.'.'.$type;
                }
            }
        }
    }
    ...
    public function fetch($template = '', $vars = [], $replace = [], $config = [])
    {
        ...
        if($this->request->param('_pop')) echo parent::fetch($this->_template, $this->_vars, $replace, $config);
        // 实例化视图并渲染
       else  return parent::fetch($this->_template, $this->_vars, $replace, $config);
    }
}
